...
class InferTensorRT:
    def __init__(
        ...
        self.engine_file_path = self.get_engine_file_path(
            onnx_file_path, engine_file_path, batch_size, mode
        )
    ...
    def get_engine_file_path(
        self, onnx_file_path, engine_file_path, batch_size, mode
    ):
        if not engine_file_path:
            engine_file_path, _ = os.path.splitext(onnx_file_path)
            engine_file_path = "{}_{}_b{}.trt".format(
                engine_file_path, mode, batch_size
            )
    ...
    def build_engine(
        ...
        with trt.Builder(self.TRT_LOGGER) as builder, builder.create_network(
            ...
            if mode == "fp16" and builder.platform_has_fast_fp16:
                ...
                calib_dataset = [
                    os.path.join(self.calib_dir, x) for x in calib_images
                ]
    ...
    def preprocess_input(self, rgb_img):
        ...
        rgb_tensor = (
            rgb_img.astype(np.float32) / 255.0 - self.means
        ) / self.stds
    ...
    def infer_img(self, img_path: str, topk: int = 3, verbose: bool = True):
        ...
        if verbose:
            print(
                f"Resized image to {(self.data_w, self.data_h, self.data_d)} from {(bgr_img.shape)}"
            )
        # resize
        bgr_img = cv2.resize(
        ...
        if verbose:
            ...
            for ind in topk_ind:
                print(
                    "{} {} ({})".format(
                        out[ind], ind, self.class_mapping[str(ind)]
                    )
                )
        return topk_ind
